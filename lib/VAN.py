...
class VAN(nn.Module):
    def __init__(self, L, depth, width, kernel_rad, device, dilations=None):
        ...
        device: 'cpu' or 'cuda',
        dilations (optional): list of dilations for each convolution, default is exponential dilation with layer number
        """
        super().__init__();
        self.depth = depth;
        self.width = width;
    ...
    def log_prob(self, sample):
        """Calculates proposal probabilitties of given samples

        Parameters:
        -----------
        sample: tensor of tensor of shape (batch_size, 1, L, L),

        Returns:
        --------
        log_prob: tensor of shape (batch_size, 1, L, L) with gradient
        """
        x_hat = self.forward(sample)
    ...
    def fast_sampling(self, batch_size):
        ...
        Notes:
        ------
        same as VAN.sample, but much more efficient. The code is really messy, but yields same results as VAN.sample if identical random seeds are chosen."""
        #torch.manual_seed(0);
